...
public class monotonicStack {
    ...
    public static int trap(int[] height){
        ...
        for(int i = left; i <= right; i++){
            while(!dq.isEmpty() && dq.getLast() < height[i]){
                int min = Math.min(height[i], dq.getFirst());
                res += min - dq.getLast();
                if(height[i] >= dq.getFirst()){
                    dq.removeLast();
                }else{
                    if(min - dq.getLast() > 0){
                        dq.removeLast();
                        cnt++;   // 对于出现当前元素大于队尾元素但是小于队首元素的情况。
                    }
                }
